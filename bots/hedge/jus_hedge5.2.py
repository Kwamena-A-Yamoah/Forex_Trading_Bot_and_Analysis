...
SL_DISTANCE_PIPS = HEDGE_DISTANCE_PIPS + 0.03 * HEDGE_DISTANCE_PIPS  # SL = 10000 + 0.3 * 10000 = 13000 pips
...
def ensure_mt5_connection():
    ...
    while True:
        ...
        send_telegram_message("❌ MT5 server disconnected, Retrying in {connect_sleep/60:.0f} minutes...")
    ...
    while True:
        ...
        if not initial_order_placed:
            ...
            if initial_order_placed and current_hedge_index < len(HEDGE_LOT_SIZES):
                ...
                if initial_take_profit and signal:
                    ...
                    if tp_hit:
                        ...
                        if isinstance(balance, float) and isinstance(previous_balance, float) and balance < previous_balance:
                            print(f"🎯 Loss at lot size: {HEDGE_LOT_SIZES[current_hedge_index - 1]}, Account balance: {balance:.2f} GHC")
                            send_telegram_message(f"❌ Loss at lot size: {HEDGE_LOT_SIZES[current_hedge_index - 1]}, Account balance: {balance:.2f} GHC")
                        else:
                            print(f"🎯 Profit at lot size: {HEDGE_LOT_SIZES[current_hedge_index - 1]}, Account balance: {balance:.2f} GHC")
                            send_telegram_message(f"🎯 Profit at lot size: {HEDGE_LOT_SIZES[current_hedge_index - 1]}, Account balance: {balance:.2f} GHC")
                ...
                if len(orders) < 1:
                    ...
                    if no_pending_order_count >= 2:
                        ...
                        send_telegram_message(f"❌ All trades closed due to no pending orders. Account Balance: {balance:.2f} GHS")
                        close_all_trades_and_orders()
                        initial_equity = mt5.account_info().equity
                        previous_balance = balance if isinstance(balance, float) else mt5.account_info().balance  # Update previous_balance
                        initial_order_placed = False
                ...
                if not active_position_exists and not tp_hit:
                    ...
                    if no_active_position_count >= 2:
                        ...
                        if isinstance(balance, float) and isinstance(previous_balance, float) and balance > previous_balance:
                            print(f"🎯 Profit at lot size: {HEDGE_LOT_SIZES[current_hedge_index - 1]}, Account balance: {balance:.2f} GHC")
                            send_telegram_message(f"🎯 No active position, Profit at lot size: {HEDGE_LOT_SIZES[current_hedge_index - 1]}, Account balance: {balance:.2f} GHC")
                        else:
                            print("❌ Two consecutive checks confirmed no active position. Closing all trades and resetting cycle.")
                            send_telegram_message(f"❌ All trades closed due to no active position. Account Balance: {balance:.2f} GHS")
                        close_all_trades_and_orders()
                        initial_equity = mt5.account_info().equity
                        previous_balance = balance if isinstance(balance, float) else mt5.account_info().balance  # Update previous_balance
                        initial_order_placed = False
            ...
            if not orders and current_hedge_index >= len(HEDGE_LOT_SIZES) - 1:
                ...
                send_telegram_message(f"❌ All trades closed due to lot size sequence exhausted. Account Balance: {balance:.2f} GHS")
                close_all_trades_and_orders()
                initial_equity = mt5.account_info().equity
                previous_balance = balance if isinstance(balance, float) else mt5.account_info().balance  # Update previous_balance
                initial_order_placed = False
                ...
                if no_pending_order_count >= 2:
                    if signal and initial_entry_price and current_hedge_index < len(HEDGE_LOT_SIZES):
                        ...
                        if hedge_order_ticket:
                            ...
                            print("❌ Failed to place new hedge order. Closing all trades and resetting cycle.")
                            send_telegram_message(f"❌ Failed to place new hedge order. Closing all trades. Account Balance: {mt5.account_info().balance:.2f} GHS")
                            close_all_trades_and_orders()
