...
HEDGE_LOT_SIZES = [0.02, 0.03, 0.06, 0.1, 0.17, 0.29, 0.5, 0.85, 1.46, 2.5, 4.29]
MAGIC_NUMBER = 10002
...
TELEGRAM_BOT_TOKEN = "8183508860:AAHLP0nXwx4ehTv2waCuTpgPUMVw2-_C22E"  # Replace with your Telegram bot token
TELEGRAM_CHAT_ID = "639750211"  # Replace with your Telegram chat ID
...
# === CHECK IF INITIAL TRADE HIT TP OR SL ===
def check_initial_trade_tp_sl_reached(initial_take_profit, initial_stop_loss, signal):
    tick = mt5.symbol_info_tick(SYMBOL)
    if tick is None:
        print(f"❌ Unable to fetch tick data for {SYMBOL}")
        return False, False
    ...
    if signal == "BUY":
        ...
        sl_hit = current_price <= initial_stop_loss
    else:  # SELL
        tp_hit = current_price <= initial_take_profit
        sl_hit = current_price >= initial_stop_loss
    return tp_hit, sl_hit

# === CHECK IF HEDGE TRADE HIT TP OR SL ===
def check_hedge_tp_sl_reached(lot_size, signal):
    positions = get_positions_with_retry(SYMBOL)
    if positions is None:
        print(f"❌ Failed to retrieve positions to check TP/SL for lot size {lot_size}")
        return False, False
    position = None
    for pos in positions:
        if pos.comment == str(lot_size) and pos.magic == MAGIC_NUMBER:
            ...
            break
    if not position:
        print(f"❌ Position with lot size {lot_size} not found for TP/SL check")
        return False, False
    tick = mt5.symbol_info_tick(SYMBOL)
    if tick is None:
        print(f"❌ Unable to fetch tick data for {SYMBOL}")
    ...
    if signal == "BUY":
        ...
        sl_hit = current_price <= position.price_open - (sl_points * point)
    else:  # SELL
        tp_hit = current_price <= position.price_open - (INITIAL_TP_PIPS * point)
        sl_hit = current_price >= position.price_open + (sl_points * point)
    return tp_hit, sl_hit
        ...
        if not initial_order_placed:
            ...
            if initial_order_placed and current_hedge_index < len(HEDGE_LOT_SIZES):
                ...
                if not active_position_exists:
                    ...
                    no_active_position_count = 0
                    last_check_time = None
            # Check if initial trade TP or SL is reached
            if initial_take_profit and initial_stop_loss and signal and current_hedge_index == 1:
                tp_hit, sl_hit = check_initial_trade_tp_sl_reached(initial_take_profit, initial_stop_loss, signal)
                if tp_hit:
                    account_info = mt5.account_info()
                    balance = account_info.balance if account_info else "Unknown"
                    print(f"🎯 Initial trade hit TP ({initial_take_profit}). Closing all trades and resetting cycle.")
                    send_telegram_message(f"🎯 TP Hit for initial trade on {SYMBOL}! Account Balance: {balance:.2f} USD")
                    close_all_trades_and_orders()
                    initial_equity = mt5.account_info().equity
                    initial_order_placed = False
                    hedge_order_ticket = None
                    ...
                    continue
                if sl_hit:
                    print(f"🛑 Initial trade hit SL ({initial_stop_loss}). Closing all trades and resetting cycle.")
                    close_all_trades_and_orders()
                    initial_equity = mt5.account_info().equity
                    initial_order_placed = False
                    hedge_order_ticket = None
                    ...
                    last_check_time = None
                    continue
            # Check if current hedge TP or SL is reached
            if current_hedge_index > 1:
                tp_hit, sl_hit = check_hedge_tp_sl_reached(HEDGE_LOT_SIZES[current_hedge_index - 1], signal)
                if tp_hit:
                    account_info = mt5.account_info()
                    balance = account_info.balance if account_info else "Unknown"
                    print(f"🎯 Hedge trade with lot size {HEDGE_LOT_SIZES[current_hedge_index - 1]} hit TP. Closing all trades and resetting cycle.")
                    send_telegram_message(f"🎯 TP Hit for hedge trade with lot size {HEDGE_LOT_SIZES[current_hedge_index - 1]} on {SYMBOL}! Account Balance: {balance:.2f} USD")
                    close_all_trades_and_orders()
                    initial_equity = mt5.account_info().equity
                    initial_order_placed = False
                    hedge_order_ticket = None
                    ...
                    continue
                if sl_hit:
                    print(f"🛑 Hedge trade with lot size {HEDGE_LOT_SIZES[current_hedge_index - 1]} hit SL. Closing all trades and resetting cycle.")
                    close_all_trades_and_orders()
                    initial_equity = mt5.account_info().equity
                    initial_order_placed = False
                    hedge_order_ticket = None
