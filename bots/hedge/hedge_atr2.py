...
HEDGE_LOT_SIZES = [0.01, 0.03, 0.07, 0.16, 0.36, 0.82, 1.86, 4.22, 9.58, 21.75, 49.42]
# HEDGE_LOT_SIZES = [0.01, 0.03, 0.08, 0.18, 0.40, 0.85, 1.80, 3.70, 7.50, 15.10, 30.30]
# HEDGE_LOT_SIZES = [0.03, 0.07, 0.16, 0.36, 0.82, 1.86, 4.22, 9.58, 21.75, 49.42]
# HEDGE_LOT_SIZES = [0.05, 0.11, 0.25, 0.57, 1.29, 2.93, 6.65, 15.1, 34.33, 78.1]
MAGIC_NUMBER = 10002
LOG_FILE = "trade_log.txt"
ATR_PERIOD = 14  # Period for ATR calculation
INITIAL_TP_ATR_MULTIPLIER = 4.0  # Multiplier for INITIAL_TP_PIPS (4x ATR)
HEDGE_DISTANCE_ATR_MULTIPLIER = 1  # Multiplier for HEDGE_DISTANCE_PIPS (2x ATR)
EQUITY_TP_ATR_MULTIPLIER = 1.2  # Multiplier for EQUITY_TP_PIPS (2x ATR)
...
def generate_signals(dataframe):
    ...
    # Calculate ATR
    atr = calculate_atr(dataframe, ATR_PERIOD)
    if atr is None:
        print(f"❌ Insufficient data to calculate ATR for {SYMBOL}")
        return None, None, None
    # Convert ATR to pips (1 pip = 10 * point for BTCUSDm)
    atr_pips = atr / point
    # Calculate pip-based parameters
    initial_tp_pips = atr_pips * INITIAL_TP_ATR_MULTIPLIER
    hedge_distance_pips = atr_pips * HEDGE_DISTANCE_ATR_MULTIPLIER
    equity_tp_pips = atr_pips * EQUITY_TP_ATR_MULTIPLIER
    ...
    if candle_2['close'] > candle_2['open'] and candle_1['close'] > candle_1['open']:
        ...
        take_profit = entry_price + (initial_tp_pips * point)
    ...
    return signal, entry_price, take_profit, initial_tp_pips, hedge_distance_pips, equity_tp_pips

# === FETCH MARKET DATA ===
def get_data(symbol, timeframe, count=10):
    rates = mt5.copy_rates_from_pos(symbol, timeframe, 0, count + ATR_PERIOD)
    if rates is None:
...
def place_hedge_market_order(signal, initial_entry_price, lot_size, hedge_index=0, hedge_distance_pips=6000):
    symbol_info = mt5.symbol_info(SYMBOL)
    if symbol_info is None:
        print(f"❌ Unable to fetch symbol info for {SYMBOL}")
        return None, None
    ...
    verify_delay = 1  # Delay in seconds for verification
...
def run_bot():
    ...
    hedge_distance_pips = None  # Store ATR-based hedge distance
    equity_tp_pips = None  # Store ATR-based equity TP
    while True:
        ...
        if not initial_order_placed:
            signal, entry_price, take_profit, initial_tp_pips, hedge_distance_pips, equity_tp_pips = generate_signals(df)
            if signal:
                print(f"📊 Using INITIAL_TP_PIPS={initial_tp_pips:.2f} (ATR x {INITIAL_TP_ATR_MULTIPLIER}), HEDGE_DISTANCE_PIPS={hedge_distance_pips:.2f} (ATR x {HEDGE_DISTANCE_ATR_MULTIPLIER}), EQUITY_TP_PIPS={equity_tp_pips:.2f} (ATR x {EQUITY_TP_ATR_MULTIPLIER})")
